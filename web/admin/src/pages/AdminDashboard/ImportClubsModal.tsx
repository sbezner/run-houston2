...
export const ImportClubsModal: React.FC<ImportClubsModalProps> = ({
  onClose,
  onImportComplete
}) => {
  ...
  const downloadTemplate = () => {
    const csvContent = 'id,club_name,location,website_url,description\n,Example Running Club,Houston TX,https://example.com,Great running club for all levels';
          ...
          <div style={baseStyle}>
            ...
            <p style={{ color: '#666', marginBottom: '20px' }}>
              {importProgress.created > 0 && importProgress.updated > 0 ? (
                <>
                  Successfully processed {importProgress.succeeded} clubs:
                  <br />
                  • {importProgress.created} new clubs created
                  <br />
                  • {importProgress.updated} existing clubs updated
                </>
              ) : importProgress.updated > 0 ? (
                `Successfully updated ${importProgress.updated} existing clubs`
              ) : (
                `Successfully created ${importProgress.created} new clubs`
              )}
              {importProgress.failed > 0 && (
                <>
                  <br />
                  ⚠️ {importProgress.failed} clubs failed to process
                </>
              )}
            </p>
            <div style={{ display: 'flex', gap: '10px' }}>
