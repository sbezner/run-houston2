...
import type { RaceReport, RaceReportsResponse } from "@shared/types";
import { Loading } from "@shared/components/Loading";
import { Alert } from "@shared/components/Alert";
import { auth } from "@shared/services/auth";
import { RaceReportForm } from "@shared/components/admin/RaceReportForm";
import { RaceReportsImportDialog } from "@shared/components/admin/RaceReportsImportDialog";
...
export const AdminRaceReportsPage: React.FC<AdminRaceReportsPageProps> = ({ onTokenExpiration }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await raceReports.delete(selectedReport.id, token);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (const reportId of selectedReports) {
        await raceReports.delete(reportId, token);
      }
            ...
            overflow: 'hidden'
               ...
               <tbody>
                 {sortReports(reports).map((report) => (
                    ...
                    <td style={{ padding: '12px', fontSize: '14px', color: '#374151' }}>
                      <div style={{ maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{report.title}</span>
                        {new Date(report.updated_at) > new Date(Date.now() - 5 * 60 * 1000) && (
                          <span style={{
                            backgroundColor: '#10b981',
                            color: 'white',
                            padding: '2px 6px',
